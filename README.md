...
10. 调色盘显示设置面板 —— 集成主题色相滑块、Banner 水波纹动画开关、顶部高光开关
11. Banner 水波纹 SVG 动画 —— 4 层叠加的动态波浪效果，平滑连接 Banner 与内容区域
12. 页面顶部渐变高光 —— 亮色/暗色自适应的渐变光效，增强视觉层次
...
导航栏中的调色盘按钮（🎨 图标）会弹出一个浮动面板，集成了以下自定义功能：

- **主题色相滑块**：拖动彩虹渐变滑块即可实时切换全站主题色（0°-360° 色相），所有颜色基于 OKLCH 色彩空间动态计算
- **水波纹动画开关**：控制 Banner 底部的 SVG 波浪动画效果
- **顶部高光开关**：控制页面顶部的渐变光效层

当主题色被锁定（`themeColor.fixed: true`）时，面板仅显示横幅设置部分。所有设置自动保存到 `localStorage`，页面刷新后保持。

**调色盘按钮显示条件：** 主题色未锁定或 Banner 已启用时，导航栏会显示调色盘按钮。

### Banner 水波纹动画

Banner 底部集成了 4 层 SVG 水波纹动画，用于平滑连接 Banner 区域与页面内容，消除硬边界的割裂感。

**特性：**

- 🌊 4 层叠加波浪，不同频率和振幅
- 🎨 颜色跟随主题色（OKLCH 色彩空间）自动适配亮色/暗色模式
- ⚡ GPU 加速动画，不影响页面性能
- 🔄 可通过调色盘面板开关控制

水波纹在 Banner 启用时自动显示，无需额外配置。

### 页面顶部渐变高光

页面顶部有一层渐变高光效果，为页面增添视觉层次和深度感。

**特性：**

- ✨ 亮色模式下为白色渐变，暗色模式下为黑色渐变
- 🎚️ 高度 180px，从上到下平滑过渡到透明
- 🔄 可通过调色盘面板开关控制
- ⚡ 固定定位，不影响页面布局
