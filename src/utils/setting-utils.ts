...
// Wallpaper video control helpers
function pauseWallpaperVideos(container: HTMLElement) {
...
function resumeWallpaperVideos(container: HTMLElement) {
	const videos = container.querySelectorAll("video");
	for (const video of videos) {
		if (video.paused) {
			video.play().catch(() => {
...
function showBannerMode() {
	...
	if (wallpaperWrapper) {
		// 移除 overlay 和 video 模式类
		wallpaperWrapper.classList.remove("wallpaper-overlay");
		wallpaperWrapper.classList.remove("wallpaper-video");
		...
		// 恢复视频播放（从 none 模式切回时视频可能已暂停）
		resumeWallpaperVideos(wallpaperWrapper);

		// 显示图片容器，隐藏视频容器
		const imagesContainer = document.getElementById("banner-images-container");
		const videoContainer = document.getElementById("banner-video-container");
		if (imagesContainer) imagesContainer.style.display = "";
		if (videoContainer) {
			videoContainer.classList.add("hidden");
			videoContainer.style.display = "none";
			// 暂停视频容器中的视频
			pauseWallpaperVideos(videoContainer);
		}
	}
...
function showVideoMode() {
	// 切换 wallpaper-wrapper 为视频壁纸模式（横幅式，在页面顶部显示视频）
	const wallpaperWrapper = document.getElementById("wallpaper-wrapper");
	if (wallpaperWrapper) {
		// 移除 overlay 模式类，添加 video 模式类
		wallpaperWrapper.classList.remove("wallpaper-overlay");
		wallpaperWrapper.classList.add("wallpaper-video");
		...
		// 恢复视频播放（考虑暂停状态）
		if (getStoredVideoPaused()) {
			pauseWallpaperVideos(wallpaperWrapper);
		} else {
			resumeWallpaperVideos(wallpaperWrapper);
		}
		...
		// 暂停图片容器中的轮播视频
		if (imagesContainer) {
			pauseWallpaperVideos(imagesContainer);
		}
	}
...
let videoCarouselTimer: ReturnType<typeof setInterval> | null = null;

/** 启动视频轮播（如果已启用） */
function startVideoCarouselIfNeeded(): void {
	stopVideoCarousel();
...
/** 停止视频轮播 */
function stopVideoCarousel(): void {
	if (videoCarouselTimer !== null) {
		clearInterval(videoCarouselTimer);
		videoCarouselTimer = null;
	}
}
...
/** 获取当前活跃的视频元素（根据屏幕宽度选择桌面或移动端） */
function getActiveVideoElement(): HTMLVideoElement | null {
	const videoContainer = document.getElementById("banner-video-container");
	if (!videoContainer) return null;
	const isMobile = window.innerWidth < 1024;
	const selector = isMobile
		? 'video[data-video-wallpaper="mobile"]'
		: 'video[data-video-wallpaper="desktop"]';
	return videoContainer.querySelector(selector);
}

/** 获取视频是否暂停的状态 */
export function getStoredVideoPaused(): boolean {
	if (
		typeof localStorage === "undefined" ||
		typeof localStorage.getItem !== "function"
	) {
	...
	return localStorage.getItem("videoPaused") === "true";
}

/** 设置视频暂停状态 */
export function setVideoPaused(paused: boolean): void {
	if (
		typeof localStorage !== "undefined" &&
		typeof localStorage.setItem === "function"
	) {
		localStorage.setItem("videoPaused", String(paused));
	}
	applyVideoPausedToDocument(paused);
}

/** 将视频暂停状态应用到文档 */
export function applyVideoPausedToDocument(paused: boolean): void {
...
/** 切换到下一个视频（在多视频列表中循环） */
export function switchToNextVideo(): void {
	if (typeof document === "undefined") return;
	...
	// 从 data 属性中获取视频列表
	const videoListStr = carrier.dataset.wallpaperVideoList;
...
/** 获取当前视频索引 */
export function getStoredVideoIndex(): number {
	if (
		typeof localStorage === "undefined" ||
		typeof localStorage.getItem !== "function"
	) {
	...
	const stored = localStorage.getItem("videoIndex");
	return stored ? Number.parseInt(stored, 10) || 0 : 0;
}

/** 设置当前视频索引 */
export function setStoredVideoIndex(index: number): void {
	if (
		typeof localStorage !== "undefined" &&
		typeof localStorage.setItem === "function"
	) {
		localStorage.setItem("videoIndex", String(index));
	}
}

/** 将指定索引的视频应用到文档 */
export function applyVideoIndexToDocument(
	index: number,
	videoList: string[],
	videoListMobile: string[],
): void {
	if (typeof document === "undefined") return;
	...
	const desktopVideo = videoContainer.querySelector(
		'video[data-video-wallpaper="desktop"]',
	) as HTMLVideoElement;
	const mobileVideo = videoContainer.querySelector(
		'video[data-video-wallpaper="mobile"]',
	) as HTMLVideoElement;

	const safeIndex = Math.max(0, index);
	if (desktopVideo && videoList.length > 0) {
		const desktopIdx = safeIndex % videoList.length;
		desktopVideo.src = videoList[desktopIdx];
		desktopVideo.load();
		if (!getStoredVideoPaused()) {
			desktopVideo.play().catch(() => {});
		}
	}
	if (mobileVideo && videoListMobile.length > 0) {
		const mobileIdx = safeIndex % videoListMobile.length;
		mobileVideo.src = videoListMobile[mobileIdx];
		mobileVideo.load();
		if (!getStoredVideoPaused()) {
...
/** 获取视频轮播是否启用 */
export function getStoredVideoCarouselEnabled(): boolean {
	if (
		typeof localStorage === "undefined" ||
		typeof localStorage.getItem !== "function"
	) {
	...
	const stored = localStorage.getItem("videoCarouselEnabled");
	if (stored === null) return false;
	return stored === "true";
}

/** 设置视频轮播启用状态 */
export function setVideoCarouselEnabled(enabled: boolean): void {
	if (
		typeof localStorage !== "undefined" &&
		typeof localStorage.setItem === "function"
	) {
		localStorage.setItem("videoCarouselEnabled", String(enabled));
	}
	if (typeof window !== "undefined") {
		window.dispatchEvent(
			new CustomEvent("videoCarouselChange", {
				detail: { enabled },
			}),
		);
	}
...
/** 获取视频列表长度（用于判断是否显示切换/轮播选项） */
export function getVideoListLength(): number {
	if (typeof document === "undefined") return 0;
	const carrier = document.getElementById("config-carrier");
	if (!carrier) return 0;
	const videoListStr = carrier.dataset.wallpaperVideoList;
	if (!videoListStr) return 0;
	try {
		return (JSON.parse(videoListStr) as string[]).length;
	} catch {
		return 0;
	}
}
