...
export const backgroundWallpaper: BackgroundWallpaperConfig = {
	// 壁纸模式："banner" 横幅壁纸，"overlay" 全屏透明，"none" 纯色背景无壁纸，"video" 视频壁纸
	// 默认为 "banner"
	...
	switchable: true,
	/**
	 * 壁纸类型："image"（默认）或 "video"（视频）
	 * 视频模式下，src 应指向 mp4 文件，多图轮播和随机功能将自动禁用
	 * 视频文件建议放在 public/videos/ 目录下，如：
	 *
	 * type: "video",
	 ...
	 * type: "video",
	 * src: {
	 *   desktop: "/videos/banner-desktop.mp4",
	 *   mobile: "/videos/banner-mobile.mp4",
	 * },
	 */
	// type: "video",
	/**
	...
	// 视频壁纸模式特有配置
	// 在壁纸模式切换面板中选择"视频壁纸"时播放此视频作为全屏背景
	// 配置后会在壁纸模式面板中出现"视频壁纸"选项
	// 视频文件建议放在 public/videos/ 目录下
	video: {
		// 视频路径，支持单个路径、多个路径数组或分别设置桌面端和移动端
		// 单个路径示例：src: "/videos/wallpaper.mp4",
		// 多视频示例：src: ["/videos/a.mp4", "/videos/b.mp4"],
		// 分端示例（支持数组）：
		src: {
			desktop: ["/videos/Cyrene.mp4", "/videos/banner-video1.mp4"],
			mobile: ["/videos/Cyrene.mp4", "/videos/banner-video1.mp4"],
		},
	},
};
