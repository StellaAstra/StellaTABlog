...
export type BackgroundWallpaperConfig = {
	mode: "banner" | "overlay" | "none" | "video"; // 壁纸模式：banner横幅模式、overlay全屏透明覆盖模式、none纯色背景或video视频壁纸
	switchable?: boolean; // 是否允许用户通过导航栏切换壁纸模式，默认true
	type?: "image" | "video"; // 壁纸类型：image图片（默认）、video视频（mp4）。视频模式下不支持多图轮播和随机切换
	src:
		| string
		| string[]
		| {
		  ...
		  }; // 支持单个图片/视频、图片数组或分别设置桌面端和移动端图片/视频
	...
	video?: {
		src:
			...
			| string[]
			| {
					desktop?: string | string[]; // 桌面端视频路径，支持单个路径或多个路径数组
					mobile?: string | string[]; // 移动端视频路径，支持单个路径或多个路径数组
			  }; // 视频路径，支持单个路径、多个路径数组或分别设置桌面端和移动端
		carousel?: {
			enable?: boolean; // 是否启用视频轮播，默认 false
			interval?: number; // 轮播切换间隔（毫秒），默认 30000
		};
	};
};
...
// 云端图床配置
export type ImageLibraryConfig = {
	defaultSpace: string; // 默认空间名称
};
