...
let videoCarouselEnabled = $state(false);
...
// 检查是否配置了视频源（从 ConfigCarrier 的 data 属性或配置中获取）
const hasVideoSrc = (() => {
...
const videoListCount = (() => {
	...
	return getVideoListLength();
})();
const hasMultipleVideos = videoListCount > 1;
...
function toggleVideoPaused() {
	...
	// 暂停时自动关闭轮播
	if (videoPaused && videoCarouselEnabled) {
		videoCarouselEnabled = false;
		setVideoCarouselEnabled(false);
	}
}
...
function toggleVideoCarouselEnabled() {
	videoCarouselEnabled = !videoCarouselEnabled;
	setVideoCarouselEnabled(videoCarouselEnabled);
	// 开启轮播时自动恢复播放
	if (videoCarouselEnabled && videoPaused) {
		videoPaused = false;
...
onMount(() => {
	...
	// 从localStorage读取视频设置状态
	videoPaused = getStoredVideoPaused();
	videoCarouselEnabled = getStoredVideoCarouselEnabled();
...
<div id="display-setting" class="float-panel float-panel-closed absolute transition-all w-80 right-4 px-4 py-2">
    ...
    {#if isWallpaperSwitchable}
        <div class="mt-2 mb-2">
            ...
            <div class="space-y-1">
                ...
                <button
                    class="w-full btn-regular rounded-md py-2 px-3 flex items-center gap-3 text-left active:scale-95 transition-all relative overflow-hidden"
                    class:opacity-60={wallpaperMode !== WALLPAPER_VIDEO}
                    class:bg-(--btn-regular-bg-hover)={wallpaperMode === WALLPAPER_VIDEO}
                    onclick={() => switchWallpaperMode(WALLPAPER_VIDEO)}
                >
                    <Icon icon="material-symbols:play-circle-outline" class="text-[1.25rem] shrink-0"></Icon>
                    <span class="text-sm flex-1">{i18n(I18nKey.wallpaperVideoMode)}</span>
                    {#if wallpaperMode === WALLPAPER_VIDEO}
                        <Icon icon="material-symbols:check-circle" class="text-[1rem] shrink-0 text-(--primary)"></Icon>
                    {/if}
                </button>
                {/if}
    ...
    {#if wallpaperMode === WALLPAPER_VIDEO && hasVideoSrc}
        <div class="mt-2 mb-2">
            <div class="flex gap-2 font-bold text-lg text-neutral-900 dark:text-neutral-100 transition relative ml-3 mb-2
                before:w-1 before:h-4 before:rounded-md before:bg-(--primary)
                before:absolute before:-left-3 before:top-1/2 before:-translate-y-1/2"
            >
                {i18n(I18nKey.videoSettings)}
            </div>
            <div class="space-y-1">
                <!-- Pause Video Toggle -->
                <button
                    class="w-full btn-regular rounded-md py-2 px-3 flex items-center gap-3 text-left active:scale-95 transition-all relative overflow-hidden"
                    class:bg-(--btn-regular-bg-hover)={videoPaused}
                    onclick={toggleVideoPaused}
                >
                    <Icon icon={videoPaused ? "material-symbols:play-circle-outline" : "material-symbols:pause-circle-outline"} class="text-[1.25rem] shrink-0"></Icon>
                    <span class="text-sm flex-1">{i18n(I18nKey.videoPause)}</span>
                    <div class="w-10 h-5 rounded-full transition-all duration-200 relative"
                         class:bg-(--primary)={videoPaused}
                         class:bg-(--btn-regular-bg-active)={!videoPaused}>
                        <div class="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-200"
                             class:left-0.5={!videoPaused}
                             class:left-5={videoPaused}></div>
                    </div>
                </button>
                <!-- Switch Video Button (only when multiple videos) -->
                ...
                >
                    ...
                    <span class="text-sm flex-1">{i18n(I18nKey.videoSwitch)}</span>
                    <Icon icon="material-symbols:chevron-right" class="text-[1rem] shrink-0 opacity-50"></Icon>
                </button>
                <!-- Video Carousel Toggle -->
                <button
                    class="w-full btn-regular rounded-md py-2 px-3 flex items-center gap-3 text-left active:scale-95 transition-all relative overflow-hidden"
                    class:bg-(--btn-regular-bg-hover)={videoCarouselEnabled}
                    onclick={toggleVideoCarouselEnabled}
                >
                    <Icon icon="material-symbols:view-carousel-outline" class="text-[1.25rem] shrink-0"></Icon>
                    <span class="text-sm flex-1">{i18n(I18nKey.videoCarousel)}</span>
                    <div class="w-10 h-5 rounded-full transition-all duration-200 relative"
                         class:bg-(--primary)={videoCarouselEnabled}
                         class:bg-(--btn-regular-bg-active)={!videoCarouselEnabled}>
                        <div class="absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-200"
                             class:left-0.5={!videoCarouselEnabled}
                             class:left-5={videoCarouselEnabled}></div>
                    </div>
                </button>
                {/if}
            </div>
